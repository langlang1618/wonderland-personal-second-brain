"""OpenAI transcript cleaning provider."""
...
class OpenAIResponsesEndpoint(Protocol):
    """Minimal OpenAI Responses API client protocol."""

    def create(self, **kwargs): ...


class OpenAIClient(Protocol):
    """Minimal OpenAI client protocol used by this provider."""

    responses: OpenAIResponsesEndpoint


class OpenAICleaningProvider(TranscriptCleaningProvider):
    """Clean transcript artifacts using the OpenAI Responses API."""

    name = "openai-cleaning"
    ...
    def clean(self, request: TranscriptCleaningRequest) -> CleaningProviderResult:
        ...
        api_key = self._api_key or _load_api_key(
            request.config.api_key_env_var,
            request.config.env_path,
        )
        if client is None:
            if not api_key:
                return _issue_result(
                    TranscriptCleaningErrorCode.MISSING_API_KEY,
                    "OpenAI API key is required for OpenAICleaningProvider.",
...
def _load_api_key(env_var: str, env_path: Path) -> str | None:
...
__all__ = ["OpenAICleaningProvider"]
